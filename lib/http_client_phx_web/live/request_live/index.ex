defmodule HttpClientPhxWeb.RequestLive.Index do
      ...
      if socket.assigns.request_id do
        case result do
          {:ok, %{body: response_body, headers: response_headers, status_code: status_code}} ->
            HttpClientPhx.Requests.update_request_response(
  ...
  def handle_event("save-request", _, socket) do
    ...
    case HttpClientPhx.Requests.create_request(attrs) do
      {:ok, request} ->
        {:noreply,
         socket
         |> assign(:saved_requests, HttpClientPhx.Requests.list_requests())
  ...
  def handle_event("load-request", %{"id" => id}, socket) do
    case HttpClientPhx.Requests.get_request(id) do
      {:ok, request} ->
        {:noreply,
         socket
  ...
  def handle_event("delete-request", %{"id" => id}, socket) do
    case HttpClientPhx.Requests.delete_request(id) do
      {:ok, _request} ->
        {:noreply,
         socket
         |> assign(:saved_requests, HttpClientPhx.Requests.list_requests())
  ...
  @impl true
  def handle_event("assign-to-collection", %{"request_id" => request_id, "collection_id" => collection_id}, socket) do
    case HttpClientPhx.Requests.update_request(request_id, %{collection_id: collection_id}) do
      {:ok, _request} ->
        {:noreply,
         socket
         |> assign(:saved_requests, HttpClientPhx.Requests.list_requests())
